<template id="">
  <div class="grid-content bg-purple">
    ...
    <update-class ref="xx"></update-class>
  </div>
</template>
...
<script>
    ...
    export default {
      ...
      activated(){//向后台请求数据
        ...
        let con = {zData:"topicDate",dDates:"topicDates"};
        con.page = this.page
        this.$store.commit("pageClickLeft",con)
          this.topicDate = this.$store.state.topicDate
      },
      //通过监听事件来监测仓库的变化
      computed:{
          //设置监听全部数据
        ...
        //设置监听分页数据
        c(){
          return this.$store.state.topicDates
        }
      },
      watch:{
          //监听全部数据
        ...
        //监听分页数据
        c(data){
          this.topicDate = this.$store.state.topicDates;
      ...
      methods:{
        handelRemove(index){
          let _id = this.$store.state.topicDates[index]._id
          this.$axios.post("/api/removeClass",{_id}).then((data)=>{
            console.log(data.data.data)
            let topicRemove = data.data.data
             this.$store.commit("topicClick",topicRemove)
            this.$message({
              message: "删除成功",
              type:"success"
            });
